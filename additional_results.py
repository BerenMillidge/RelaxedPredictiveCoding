...
if __name__ == "__main__":
    ...
    act_fns = ["relu","tanh"]
    layer_sizes = ["100_20_20","200_50_50","300_100_100","500_200_200"]
    for act_fn in act_fns:
        for layer_size in layer_sizes:
            basepath = bpath + str(act_fn) + "_" + str(layer_size) + "__"
            default_path = basepath + "default"
            learnt_error_connections = basepath + "error_connections"
            nonlinearities_path = basepath + "no_nonlinearities"
            combined_path = basepath + "full_construct"
            plot_results(default_path, nonlinearities_path,act_fn + " LS Without Backwards Nonlinearity", "Standard Predictive Coding", "Without Nonlinear Derivative",dataset=dataset,act_fn="relu")#,path3=fa_path,label3="Feedback Alignment")
            plot_results(default_path, learnt_error_connections,act_fn + "LS With Full Error Connectivity", "Standard Predictive Coding", "With Error Connections",dataset=dataset,act_fn="relu")
            plot_results(default_path, combined_path,act_fn + "LS Combined Algorithm","Standard Predictive Coding", "Combined Relaxations",dataset=dataset,act_fn="relu")

    """
    # weight clamp
    bpath = "relaxed_pc_experiments/weight_clamp_"
    act_fns = ["relu","tanh"]
    weight_clamps = [1,2,10,20]
    for act_fn in act_fns:
        for weight_clamp in weight_clamps:
            basepath = bpath + str(act_fn) + "_" + str(weight_clamp) + "_"
            default_path = basepath + "default"
            learnt_error_connections = basepath + "error_connections"
            nonlinearities_path = basepath + "no_nonlinearities"
            combined_path = basepath + "full_construct"
            plot_results(default_path, nonlinearities_path, act_fn + "WC Without Backwards Nonlinearity", "Standard Predictive Coding", "Without Nonlinear Derivative",dataset=dataset,act_fn="relu")#,path3=fa_path,label3="Feedback Alignment")
            plot_results(default_path, learnt_error_connections,act_fn + "WC With Full Error Connectivity", "Standard Predictive Coding", "With Error Connections",dataset=dataset,act_fn="relu")
            plot_results(default_path, combined_path,act_fn + "WC Combined Algorithm","Standard Predictive Coding", "Combined Relaxations",dataset=dataset,act_fn="relu")
